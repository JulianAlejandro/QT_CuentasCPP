...
int main(int argc, char *argv[])
{
    ...
    // 1. Insertar una categoría de gasto
    qDebug() << "\n1. Insertando categoría de gasto...";
    if (sqlManager.insertarCategoria("Comida", 1, "gasto") > 0) {
        qDebug() << "✓ Categoría 'Comida' (gasto) insertada correctamente";
    } else {
        qDebug() << "✗ Error insertando categoría";
    }

    // 1b. Insertar una categoría de ingreso
    qDebug() << "\n1b. Insertando categoría de ingreso...";
    if (sqlManager.insertarCategoria("Salario", 0, "ingreso") > 0) {
        qDebug() << "✓ Categoría 'Salario' (ingreso) insertada correctamente";
    } else {
        qDebug() << "✗ Error insertando categoría";
    }
